...
public class Volume_xCollection  implements Ix_DirCollection{
	...
	*/
	...
	/**
	 * Ajoute un élément à notre container
	 * on ajoute un élément de type VolumeData, construit à partir d'un objet File
	...
	public int AddItem(int index_parent, File file) {
		...
		item.setRef(index);
		//System.out.println( item.isDirectory() ? "Dir": "File");
	...
	public ArrayList<Ix_DirItem> getAllDir() {
		...
		while (it.hasNext()) {
			...
			if (item.isDirectory()){
				ret.add(item);
				System.out.println( "Dir -> " + item.toString());
			}
		}
		return ret;
	}
	...
	public ArrayList<Ix_DirItem> getAllFiles() {
		...
		while (it.hasNext()) {
			...
			if (!item.isDirectory()){
				ret.add(item);
				System.out.println( "File -> " + item.toString());
			}
		}
		return ret;
	}
